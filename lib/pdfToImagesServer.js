...
// Importar canvas con manejo de errores para entornos donde no esté disponible
let createCanvas;
try {
  const canvasModule = require('canvas');
  createCanvas = canvasModule.createCanvas;
} catch (canvasError) {
  console.warn('[pdfToImagesServer] Canvas no disponible:', canvasError.message);
  createCanvas = null;
}
...
export async function pdfToImagesServer(pdfBuffer, scale = 1.5) {
  ...
  if (!createCanvas) {
    throw new Error('El módulo canvas no está disponible. Las imágenes se generarán en el cliente.');
  }
